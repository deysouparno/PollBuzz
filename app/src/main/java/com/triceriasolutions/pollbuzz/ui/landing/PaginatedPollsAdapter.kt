...
class PaginatedPollsAdapter(private val listener: ClickListener) :
    PagingDataAdapter<Poll, PaginatedPollViewHolder>(Companion) {
    ...
    override fun onBindViewHolder(holder: PaginatedPollViewHolder, position: Int) {
        getItem(holder.bindingAdapterPosition)?.let { holder.bind(it) }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PaginatedPollViewHolder {

        val binding =
            PollItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)

        val viewHolder = PaginatedPollViewHolder(binding)
        ...
        binding.apply {
            ...
            if (poll.created) {
                ...
                FirebaseFirestore.getInstance().collection("following")
                    .document(FirebaseAuth.getInstance().currentUser!!.uid)
                    .collection("followingUsers").document(poll.creatorId)
                    .get().addOnSuccessListener {
                        if (it.exists()) {
                            poll.following = true
                        }
                        Log.d("adapter", "following:- ${poll.following}")
